...
Public packages are read-only contracts. Two execution profiles ship: `http_request`, a static declared request whose only dynamic parts are caller arguments; and `browser_page_script`, the sole source-bearing profile — exact source bytes reviewed by a maintainer, covered by the signed manifest, and executed only in a fresh sandboxed browser page under signed same-origin egress, request, byte, and structural-result limits. Packages cannot install or run Node modules, execute unreviewed source, choose arbitrary network destinations, bypass the browser/security boundary, or turn a scrape into hosted execution. The registry is signed static metadata; target-site execution and output stay local.
...
The exporter reads the saved strategies itself. The review input supplies only the public contract that discovery cannot safely infer: package/catalog metadata, typed structural outcomes, exact browser egress and traffic budgets, reviewed page-script settings, and one to eight grounded fixtures per capability. An installable release's `catalog.runtime_range` must include the runtime performing the export, because that runtime is the one that smoke-verifies the fixtures. Each fixture entry is discriminated by `kind`: a `call` fixture is `{fixture_id, kind: "call", input}`; a `run` fixture is `{fixture_id, kind: "run", input, caller_bounds, input_mode_id}`. Every capability needs at least one call fixture, and a capability that declares a `collection` also needs at least one run fixture; run fixtures on capabilities without a collection are rejected. Every reviewed capability must be one of the platform's active saved capabilities — active capabilities left out of the review stay local — and each reviewed capability must have an active strategy at its reviewed tier whose post-save semantic verification passed.

Each reviewed capability declares exactly one strategy block, and the key names the tier: `page_script` (`{tier: "page-script", strategy_id, wait, interaction, expect, request_body_limits, replay}`) exports a saved local `page-script`; `http` (`{tier: "fetch", strategy_id, context, replay, response_body_limit_bytes}`) exports a saved local `fetch`. Declaring both, or neither, is rejected.

An `http` export carries the local `response` over as the public projection: a JSON response projects as-is, and an HTML response with `extract` selectors becomes an `html` projection with the same selectors, attributes, multiplicity, and row fields, so the strategy exported is the strategy that was verified. Two response constructs are refused by name: a `from` binding (the result comes from a prerequisite) and a `json` leaf (a dot-path read of script text); both belong at the page-script tier. It compiles the local request's `{{name}}` templates into declared value expressions, and every placeholder must name a top-level property of the reviewed `input_schema` — a placeholder the contract does not declare would otherwise bake the author's own value into every caller's request. Two local constructs have no public equivalent and are refused by name rather than dropped, because a strategy exported without its dynamic parts is not the strategy that was verified: `generated` values (arbitrary local JavaScript, e.g. `{{__gen.timestamp}}`) and non-empty `prerequisites` (which need a live page or a prior request). A capability that needs either exports at the page-script tier, where the program runs inside the page.
...
On a `map` session, `lift_observed_capability({session_id, name, args?})` opens a triage+lift cycle for one slug that already exists on `platform_logbook.observed_capabilities[]` (i.e. the agent called `record_observed_capability` earlier this session or a prior one). The runtime adds the slug to `session.declaredCapabilities` and transitions the FSM to triage. The agent then runs the normal triage → `submit_triage_plan` → lift → `save_strategy` flow. To include another observed slug before the final save, call `lift_observed_capability` from lift; it returns the FSM to triage for the new slug. A final save closes the session, or the agent can close explicitly with `end_drive`.
...
Field-level schema renders live in every `save_strategy` rejection envelope and on the `submit_triage_plan` ok-response. When a session expires, `execute` returns `{needs_reauth: true}` instead of a raw 4xx.
...
- Each extract entry: `{selector, attr?, multiple?}` (leaf), `{selector, json, multiple?}` (JSON leaf), **or** `{selector, multiple, fields: {...}}` (row group). CSS selectors only. Absent `attr` pulls trimmed `textContent`.
- **JSON leaves** read data the site server-renders into a `<script>` tag rather than into markup — `__NEXT_DATA__`, `__NUXT__`, `application/ld+json`, per-app rehydration blobs. `json` is a path into the parsed text (`"props.pageProps.item.price"`, `"items[0].id"`); `""` returns the whole document. **Bracket-quote any key containing a dot** — `__DEFAULT_SCOPE__["webapp.user-detail"].userInfo` — since rehydration blobs commonly namespace scope keys that way and an unquoted key gets split into segments that resolve to `""`. Values come back as **raw JSON** — numbers stay numbers, objects stay objects — so the exact server-side value survives instead of the rounded, locale-formatted one the markup shows. Prefer this over a DOM selector whenever both are present: `422500` from the blob beats `"422.5K"` from the heading. Unparseable text or an unresolved path yields `""`, so the all-empty probe still fires. `json` is mutually exclusive with `attr` and `fields`; it also works inside a row group's `fields` (scoped to that row).
...
- **`html_extract_returns_json_blob`** — the probe flags any leaf without `json` whose extracted value is a string that parses as a JSON object or array: the extract read a `<script>` tag's text and is handing the caller the whole serialized document to re-parse, which can exhaust the response budget by itself. Decided on the probe's actual bytes, not on how the selector is written, so `{selector: "#__DATA__"}` is caught too. Add a `json` path to each named entry, or ack when returning the raw document is deliberate: `notes.save_warnings_acked: [{kind: "html_extract_returns_json_blob", reason: "<why>"}]`.
...
Use when the same HTTP call works in the browser but returns 403/429 from Node (PerimeterX, Cloudflare, Akamai, DataDome) AND the call doesn't need a page-extracted token — if it does, save as `fetch` with `transport: 'browser'` instead. The runtime navigates to `origin`, then runs `fetch()` via `page.evaluate`. Field-level schema renders live in `save_strategy` rejections + the `submit_triage_plan` ok-response.
...
### Amending a saved fetch / page-script strategy (`update_strategy`)

`patch_step` heals recorded-path _steps_. To change a saved **fetch / page-script** strategy's body — fix a bug in the expression, or enrich it (e.g. extract more response fields, add a prereq) — use `update_strategy({platform, capability, strategy, session_id})`. It loads the saved strategy, applies your **full replacement** body (load the current one via `get_strategy`, edit, resubmit), and re-runs the **same** save-time audit + probe + commit as `save_strategy` (including `user_confirmation`) — an amend is held to a create's bar, so hand-editing the JSON is never the move. It rejects if no saved strategy exists yet (that's `save_strategy`'s job: discover → lift → save). Admissible from drive / execute / lift — you don't re-enter discovery to improve a healthy strategy. The session supplies the live audit context + the pool the post-save probe needs, so `session_id` is required. Triage-coupled gates don't re-fire (the surface was triaged at create time); the payload-level gates (literal_provenance, enum grounding, mutating verification, user_confirmation) run against the amended body.
...
4.  **For `recorded-path` strategies**: use `patch_step({platform, capability, strategy_type, step_id, patch: {locators}})` to fix just the broken step's locators without rewriting the rest — the fields to change go in the `patch` object, not as top-level args. Your `patch.locators` REPLACES the step's locator cascade by default; pass `patch: {locators, merge_locators: true}` to shallow-merge onto the existing cascade and keep alternatives you didn't mention. The `recorded_step_failed` checkpoint payload names the `failed_step_id` directly. See `klura://reference#step-healing-response-format`.
...
3. **User-arbitration is at save time, not on a runtime cadence.** Every `save_strategy` runs through the `user_confirmation` classifier (see [`#save-strategy-audit`](#save-strategy-audit) above). The first call returns `items.user_confirmation.required_facts` — a struct with the load-bearing facts about the proposed save (capability, tier, target, anchor_type, warning_kinds) — alongside an `agent_note` re-stating the freshness contract. Compose a 1-3 sentence prompt in your own voice that mentions every fact, ask the user, and retry with `audit_answers.user_confirmation: {agent_prompt: "<the prose you showed them>", user_decision, user_quote: "<their fresh reply>"}`. The runtime structurally checks `agent_prompt` names the crisp facts (capability slug, tier, target host); the anchor classification and open warning kinds ride `required_facts` for you to relay in your own words / language (no keyword check). **Do NOT reuse the user's reply to a prior `ack_checkpoint` (triage_plan, surface_changed) or any earlier turn** — the runtime cannot detect recycled replies; freshness is on the agent. Reject keeps the session in the current phase — try a different tier or anchor based on the user's reason.
