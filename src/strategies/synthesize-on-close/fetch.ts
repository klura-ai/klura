 ...
 *  warm execute might 4xx because the saved value won't validate from
 *  a Node-side context. Cheap structural heuristic — false positives
 *  are advisory-only (the agent reads the diagnostic and decides),
 *  false negatives are fine because real failures will surface as 4xx
 *  on warm execute anyway.
 *
 *  Skip standard non-secret headers (Content-Type, Accept, etc.) and
 *  any header value that contains an `{{...}}` template (agent's args
 *  drove it). Flag values that are >= 24 chars and have mixed character
 *  classes consistent with random bytes (token, hash, signature).
 */
function detectOpaqueHeaderTokens(
  ...
  const argValues = new Set<string>(Object.values(args));
  const out: string[] = [];
  for (const [name, value] of Object.entries(headers)) {
    if (SAFE_HEADER_NAMES.has(name.toLowerCase())) continue;
    if (typeof value !== 'string' || value.length < 24) continue;
    if (value.includes('{{')) continue; // already templated against an arg
    if (argValues.has(value)) continue; // verbatim agent arg
    // Mixed-class entropy proxy: has letters + digits AND at least one
    // non-alphanumeric or mixed case. JSON / readable text usually fails.
    const hasLetter = /[A-Za-z]/.test(value);
    const hasDigit = /\d/.test(value);
    const hasNonAlnum = /[^A-Za-z0-9]/.test(value);
    const hasMixedCase = /[a-z]/.test(value) && /[A-Z]/.test(value);
    if (!hasLetter || !hasDigit) continue;
    if (!(hasNonAlnum || hasMixedCase)) continue;
    // Skip values that look like a URL — agents sometimes pass URLs verbatim
    // in headers (Referer-shape), and URLs trip the entropy heuristic.
...
export function synthesizeFetchFromCaptures(
  ...
  for (let i = 0; i < saves.length; i += 1) {
    ...
    try {
      ...
      // Context-bound token diagnostic: when the saved strategy templates
      // headers whose values look opaque (high-entropy, not derivable from
      // agent args) the server may be validating tokens that bind to the
      // JS-execution context that generated them. Warm execute templates
      // these as static — if the binding is strict, the warm call 4xxs.
      // Surface the third RE-toolkit axis as the lift path. Parallel to
      // `literal_in_ws_frame_only`. See `runtime/src/tools/context-bound-eval.ts`.
      const opaqueHeaderNames = detectOpaqueHeaderTokens(templatedHeaders, save.args);
      if (opaqueHeaderNames.length > 0) {
        diag.push({
          pass: 'synth_fetch',
          capability: save.capability,
          phase: 'save',
          outcome: 'context_bound_token_in_request',
          detail: {
            header_names: opaqueHeaderNames,
            advice:
              'Captured request includes header(s) whose values were generated by page-script and may bind to the JS context that generated them. If warm execute returns 4xx with these templated as static, lift manually via `evaluate_in_iframe` / `evaluate_in_iframe_chain` / `evaluate_in_worker` — third RE-toolkit axis, parallel to `inspect_ws_frame` + `try_generator` for binary WS.',
          },
        });
      }
    } catch (err) {
