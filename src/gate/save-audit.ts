      ...
      return matches.map(
        (m) =>
          `literal_provenance["${item.path}"] = "static" but the value contains ${JSON.stringify(
            ...
            m.value,
          )} with {{${m.paramName}}} in the field, declare notes.params.${m.paramName} as {kind: "enum", observed_values: [{value, label}, ...]} grounded in the captured pair, and reclassify this entry as {caller_input: "${m.paramName}"}. See klura://reference#enum-grounding.`,
      );
    }
    return [];
  }
