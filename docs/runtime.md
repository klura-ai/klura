...
- **`@klura/driver-playwright-stealth`** (separate package) — install alongside `klura` and set `pool.driver` to the package name. Extends `PlaywrightDriver` to correct automation artifacts a real Chrome session doesn't have, across three layers:
  - **DOM-level** (`puppeteer-extra-plugin-stealth` via `playwright-extra`): `navigator.webdriver`, `chrome.runtime`, plugin enumeration, WebGL vendor/renderer strings, language/platform consistency.
  - **CDP-level** (`rebrowser-playwright` drop-in): suppresses the `Runtime.enable` execution-context leak that lets a page observe it's being driven over the DevTools Protocol. This leak lives in the protocol client, not in any page-visible property, so DOM patches can't reach it — it's the signal challenge widgets lean on hardest. Also neutralizes the `window.__pwInitScripts` main-world global (Playwright's init-script dedup guard) by keying it off a per-process-random name via an idempotent postinstall patch.
  - **Context-level**: substitutes a realistic desktop viewport when the caller didn't pin one, so the context never ships Playwright's tell-tale default `1280×720`.

  Each layer self-verifies at load and warns loudly if a fix isn't armed (a silently-degraded stealth driver is worse than none). Kept out of the main runtime so users who don't need stealth don't pay the install cost.
...
**Connect mode** (`pool.connect`) is the built-in version of the `connectOverCDP` shape: instead of Playwright _launching_ the browser, the driver attaches to a **normally-launched** Chrome (spawned by klura with a persistent profile, or one you started with `--remote-debugging-port`). A normally-launched Chrome clears managed browser challenges that a Playwright-launched one fails — the distinguishing signal is the launch profile, not the CDP connection. See [drivers.md#connect-mode](drivers.md#connect-mode).
