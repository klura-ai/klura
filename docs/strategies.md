...
**Cookie propagation requires warm-pool mode.** `pool.warm.enabled: true` in `~/.klura/config.json` enables the `BrowserContext` reuse that makes side-effect prereqs work. In cold-pool mode each sub-execute gets a fresh context and side-effect-only prereqs silently fail to share state — confirm warm pool is on, or fall back to a binding-oriented prereq (extract the session token from the login response, bind it as `{{session_token}}`, dependents send as a header) which works in either mode. Runtime-owned post-save verification carries its own run-scoped equivalent: one context per `(platform, identity)` for the whole verification run, so a side-effect prereq is honoured there too (see [validation.md](./validation.md#verification-context)).

**Save-time advisory.** A no-`vars` capability/tag prereq whose target is not a pure read raises `side_effect_prereq_unproven`: nothing is bound, so the runtime holds no structural evidence the effect landed, and a consent click that silently missed looks identical to one that worked. Bind a value that only exists post-effect, fold the step into the strategy's own prereq chain, or ack with a reason when the effect genuinely leaves no trace in the target's response. Its complement, `useless_capability_prereq`, fires when the same shape targets a confirmed pure read (GET, no `provides`) — exactly one of the two applies to any saved, resolvable target.
...
A separate per-protocol counter (`NODE_TRANSPORT_FAIL_THRESHOLD = 3` in `runtime/src/strategies/health.ts`) persistently demotes a `fetch` strategy to `page-script` after three consecutive transport-layer failures (TLS fingerprint mismatches, ECONNRESET on Node-side requests). This is finer-grained than the cascade above — it preserves the same capability while changing the execution tier — and the recorded-path is never deleted after graduation; it serves as the ultimate fallback. Each strategy tier has independent health tracking (see [health.md](health.md)). After 5 consecutive failures, a strategy is archived as broken (`BROKEN_THRESHOLD = 5`); use `reset_strategy_health` to restore it.
