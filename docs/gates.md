...
| `html_extract_returns_json_blob` | 2 | `runtime/src/strategies/probe/fetch-html.ts` | Probe-stage, so the verdict rests on the bytes the site actually returned rather than on how the selector reads — an id-only selector onto a `<script>` tag looks like any other selector. Cleared by a `notes.save_warnings_acked` entry, same channel as `save_probe_target_crashed` in `probe/js-eval.ts`. Ackable because returning a whole document verbatim is occasionally the intent. |
