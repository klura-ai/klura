...
Every LLM-supplied artifact (strategy file, prereq selector, header value, endpoint URL, platform slug, identity key, policy tier) goes through `runtime/src/validators/index.ts` at save time. Save-time DOM probing for `page-extract` selectors lives in `runtime/src/strategies/probe.ts` — it catches "agent invented a selector that doesn't exist" before the strategy ever lands on disk. See [validation.md](validation.md) for the full save-time validation pipeline.
...
The exception worth naming: don't pre-compute expensive things on every tool response "just in case." Scope the enforcement to the one decision point that actually needs it. The minimal triage bundle (current_tier + prior_attempts + discovery_artifact) inlines on end_drive's LIFT handoff because that's the LIFT decision point — not on every `perform_action` response. Runtime enforcement that bloats every response is its own anti-pattern; the useful shape is "auto-invoke at the exact decision point the result is needed, nowhere else."

**If you do need a prompt reminder, put it as close to the run as possible, not in front.** A line in `runtime/SKILL.md` that says "after `end_drive`, call X" sits at the top of every conversation and still gets skipped when the context is crowded. The same reminder injected _into the `end_drive` response itself_ — attached to the specific tool result that precedes the action — lands at the decision point where the agent is already reading. Prefer the inline injection to the front-loaded rule.
...
4. **One source of truth for schema shapes and enum prose.** Corollaries #2 and #3 only hold if the rejection prose stays in sync with what the executor actually accepts. Two surfaces that describe the same set — the validator's accept check AND the rejection's skeleton / allowlist — WILL drift as fields are added, renamed, or removed; every change becomes a two-place edit and sometimes one gets missed. The canonical pattern has two layers. **Identifier sets** (tier names, prereq kinds, session outcomes) live as `const`-as-union arrays in `runtime/src/vocab/index.ts` (`STRATEGY_TIERS`, `PREREQ_KIND_VALUES`, `SESSION_OUTCOMES`); every registry keyed by one carries `satisfies Record<...>` so tsc enforces exactly-one-entry-per-value, TS unions derive via the vocab type, Zod enums via `z.enum(STRATEGY_TIERS)`, and MCP JSON-schema enums via `[...STRATEGY_TIERS]`. **Field shapes** live as Zod schemas (`runtime/src/strategies/schemas/`); the validator runs them via `parseOrThrow` and the rejection skeleton renders from the same schema via `describeShape` / `renderZodSkeletonInline` / `renderZodSkeleton` (`runtime/src/strategies/schemas/zod-helpers.ts`), with `describeEnum` (`runtime/src/validators/index.ts`) for small enum lists in `asEnum` rejections. Adding a field or a vocabulary value is a one-place change. Red-flag shape to grep for and eliminate: a string literal that lists fields (`"a", "b", "c"`) or bullets a schema inside an error message — almost always means the canonical set lives elsewhere and is being re-typed by hand.
...
- **Schema shapes and enums have one source of truth.** The per-kind prereq schemas (`prereqSchemas` in `runtime/src/strategies/schemas/prereqs.ts`) and per-tier strategy schemas (`strategySchemas` in `runtime/src/strategies/schemas/strategy.ts`) are the canonical examples: one Zod declaration per kind/tier drives the validator (`parseOrThrow`), the rejection skeleton (`renderZodSkeleton`), and the agent-facing schema catalog (`runtime/src/strategies/schema-catalog.ts`). Both registries are keyed by the vocab (`PREREQ_KIND_VALUES` / `STRATEGY_TIERS` in `runtime/src/vocab/index.ts`) with `satisfies Record<...>` coverage — adding a prereq field is a one-line schema edit; adding a kind or tier starts as a one-entry vocab edit that tsc then walks through every registry and union. The `notes` allowlist applies the same pattern via `describeNotesAllowlist` (`runtime/src/strategies/validate/notes.ts`).
...
- **`save_strategy` ack-gates structural save-time warnings.** The runtime emits warnings for detected red flags — `unparametrized_session_id` (expression bodies reading session-scoped state and extracting via `.match` / `.split` / `.slice`; suppressed when the body reads caller `args.` or a capability/tag prereq is declared, because string-shape matching cannot distinguish a caller-derived value with a page-state fallback from a session-pinned value), `unresolved_name_to_id_gap` (`notes.params.X.example` is id-shaped but the caller's declared args contain no matching `X` and no capability-prereq binds to it), `entity_pinned_infra_prereq` (a `prerequisites[i].url` has a path segment or query value equal to a value from `session.declaredCapabilities[0].args`; hostname matches are excluded), and several others (see [gates.md](gates.md)). Unacked warnings block the save with a multi-bullet rejection naming kind + message + hint for each. Agent unblocks by either fixing the strategy (save a lookup sibling, use a site-root URL, thread a `{method:"capability"}` prereq) or adding `notes.save_warnings_acked: [{kind, reason}]` with a one-sentence justification per kind. Matches corollary #1 (agents can't override strong priors from advisory warnings alone — a SKILL.md sentence is ignorable, a rejection at the decision point is not) + the "Prefer runtime enforcement" principle below. The detector functions live in `runtime/src/gate/save-warnings.ts`; the consolidated `Audit` (`runtime/src/audit/lift/save-strategy.ts`) wraps each as a `Detector` spec and reconciles `notes.save_warnings_acked`.
...
Shared budget helpers live in `runtime/src/response/response-size.ts`. Reuse them:
...
- `enforceFinalBudget(value, {ceiling, toolName})` — last-resort transport-cap backstop. Per-tool helpers compact shape-aware (they know `status` and `body.ok` are decision signals); this one is shape-blind and runs after, walking the result tree depth-first and greedy-truncating the largest leaves (strings ≥ 256 chars via `truncateString`, arrays ≥ 50 entries head-sliced with a `{__truncated, original_length, kept}` sentinel) in a fixed-point loop until the serialized result fits the ceiling. Returns `{value, truncations: [{path, from, to}]}`; the caller injects a top-level `_runtime_oversize_warning` when truncations are non-empty so a regression in the per-tool layer is loud in field reports. **The two layers compose by design** — per-tool keeps the agent-meaningful structure (success markers, hints) intact at typical sizes; backstop keeps the MCP transport ceiling unreachable when the per-tool layer misses a shape. Wired in `formatToolResult` (`runtime/src/public-api.ts`) at the `MAX_TOOL_OUTPUT_CHARS` ceiling.
- `compactExecuteResultBody(executeResult)` (`runtime/src/tools/start-session.ts`) — the `start_session` auto-execute compactor. It reserves a 3 KB share for `execute_result.body`, renders line separators inside body strings as visible escapes, clips long string leaves while retaining the structured body, and reports `body_total_chars`, `body_truncated`, and `body_truncated_paths`. This keeps identifiers and typed outcome fields readable even when result items contain multiline captions or signed media URLs. Execute-only sessions omit discovery artifacts, the platform-map teaser, and recent abort history; callers can open a discover/map session when they need those authoring surfaces.
...
- `formatToolResult` (`runtime/src/public-api.ts`) — extracts base64 screenshots into separate MCP image blocks so they don't inflate the JSON-text block.
...
3. **`runtime/src/response/data-load-classifier.ts`** — pattern-matches the "page's data-load XHR" shape (same-origin + JSON + list-shaped body + name-affinity to capability tokens). Drives the `data_load_candidates` end-drive review for read-only capabilities discovered without typed-literal args.

4. **`unparametrized_session_id` save-warning detector** (`runtime/src/gate/save-warnings.ts`) — structural match on `SESSION_STATE_READS` (`runtime/src/strategies/validate/constants.ts`) (e.g. `location.href`, `document.URL`) combined with `ID_EXTRACTION_SHAPES` in generator / prereq code. Catches the "id read from whatever page the session happens to be on, not portable across recipients" failure mode.

5. **`OPAQUE_EXAMPLE_PATTERNS`** (`runtime/src/strategies/validate/constants.ts`, consumed by `validate/opaque-params.ts`) — anchored regex bank for opaque-token shapes (prefixed internal IDs like `R_kgDO...`, URI-scheme handles like `gid://...`, long hex blobs, UUIDs, ULIDs, base64-shaped blobs ≥30 chars). Runs against `notes.params[].example` values only: a declared-as-user-typed example matching one of these shapes is almost certainly a misclassified prereq.

6. **Session-observation provenance** (`runtime/src/response/observation-trace.ts`, consumed by the save-strategy audit's `observed_property_keys` and `observed_literal_values` Detectors) — the runtime records strings the agent saw via tool responses (`js_eval` results, `find_in_page` matches) into a per-session `observedStrings` set. At save time, the audit cross-references prereq expression keys and strategy header/body/step values against that set, filtering small allowlists for DOM/JS standards (`STABLE_API_NAMES`) and HTTP wire vocabulary (`STABLE_LITERAL_VALUES`). Remaining matches mean the agent baked a name or value they only know because they saw it _this session_ — fragile by definition (observation = "what this build calls this thing," not "what web standards call this thing"). Crisp by construction: set membership, no language assumptions, no prose match. _Empirical: removing the expression-key half regressed `llm-tests/scenarios/drift-offsets` — V1 reverted to baking observed path literals and V3 drift-recovery cost ~3× more rounds._
...
8. **Tracking-shape URL path filter** (`TRACKING_PATH_PATTERNS` in `runtime/src/audit/drive/end-drive.ts`) — anchored regex bank on URL pathname segments matching industry-standard tracking/telemetry conventions (`/collect`, `/beacon`, `/pixel`, `/track`, `/metrics`, `/rum`, `/heartbeat`, `/analytics`, `/telemetry`, `/events`). Used by the `unsaved_xhr_endpoints` Detector to keep its rejection envelope readable when the captured network log mixes capability-shaped XHRs with first-party telemetry. Crisp: anchored regex on path segments, no brand-specific hostnames, no fuzzy keyword matching against response bodies. False positives are tolerable — the rejection envelope lists the surviving paths and the agent acks-with-reason if a real capability slipped through. Tradeoff: filter is conservative on purpose (no brand-specific hostname allowlists like `googletagmanager` / `doubleclick`) because hostname banks rot faster than path-segment conventions; brand-specific filtering should happen via the structural `/collect` / `/beacon` shapes those vendors emit anyway. PRs adding hostname-specific patterns are auto-reject per the "regex on specific site hostnames" rule above.

9. **Recurring-read graduation advisory** (`collectRecurringCoveredReads` in `runtime/src/audit/drive/xhr-noise.ts`, surfaced as `recurring_read_advisory` on the `end_drive` ok-response) — buckets the captured 2xx network log by `method + origin+pathname` and flags paths hit ≥ 3× this session that a SIBLING saved capability already path-covers (so the blocking `unsaved_xhr_endpoints` gate stays silent) but that weren't graduated this session. This is the read-only-js_eval gap: an agent can re-derive the same search via js_eval every session on a covered gateway (e.g. `/sch/i.html` that many distinct operations multiplex onto) without any gate nudging a save. Crisp throughout: count of distinct hits on the same path-template (no fuzzy matching), covering-capability named via the same coverage regex the gate uses. Deliberately NON-blocking and advisory-only — whether the recurring read is a distinct capability worth saving or a one-off on a covered gateway is a semantic judgment the runtime can't make, so it surfaces the fork (`execute(<covering capability>)` vs save a sibling) and lets the LLM decide. Uncovered recurring reads are excluded on purpose: the blocking gate owns those. Fires conservatively (≥ 3 hits, covered, not-graduated-this-session) so search-light sessions see nothing.
