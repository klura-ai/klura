...
- **Intent up front.** `start_session(url, {platform, capability, args})`, `declare_capability`, or map-mode `lift_observed_capability` names the capability slug and preserves the caller's input values before authoring. Values may be strings, numbers, booleans, arrays, objects, or null. Scalar strings remain eligible for capture-literal templating; structured values remain available to authored strategies through own-property placeholders such as `{{queries.0}}`. Without a declaration, auto-save has no slug to key under and no param names to template — the fallback is a recorded-path-only save.
- **Auto-execute at start_session.** If a complete strategy already exists for `{platform, capability}`, `start_session` runs it in-session and returns `executed: true` with the result. A boolean `execute_result.body.ok` is an explicit local result; HTTP 2xx without it is transport-only, so the agent inspects the typed body before reporting success. `executionState:"not_run"` means no target request was sent, while `"sent_unconfirmed"` means a write may already have reached the target; neither state is retried or treated as strategy health evidence.
- **Safe-read candidate review.** A new structurally safe read executes from an inactive hash-addressed candidate. `body.ok:true` promotes immediately. A normal 2xx payload remains inactive until the LLM reads the exact digest-bound evidence through `review_strategy_candidate` and submits `verified_success`; failure or uncertainty stays inactive. The runtime verifies the binding and never interprets response or rationale text.
- **Exact verification proof.** The runtime stamps successful verification with platform, capability, exact tier, canonical strategy digest, build provenance, and a stable verifier-contract version. Commit plus proof capture, success stamping, and failure archival each compare and mutate under the capability lock. A concurrent save therefore makes an older result stale instead of attaching it to or archiving the newer bytes. Save, candidate promotion, public export, and actor grading use one shared assessor. A changed strategy or sibling tier cannot reuse the proof, while an unrelated runtime rebuild alone does not expire it. A later explicit-success execution refreshes the proof for the exact executed bytes without another model round trip.
- **The `execute` graph.** `start_session({graph: "execute", platform, capability, args})` makes the saved-strategy invocation the whole session. An explicit success or transport-complete response terminates `closed`, but transport completion is not a semantic outcome claim. A local strategy's explicit `body.ok:false` is structurally repairable: unless a stronger typed diagnosis says authentication has expired, the live session enters triage on the first failure so the LLM can inspect the typed body and page, submit a surface-bound plan, and update the strategy. The runtime never interprets an application-defined `code` or error prose. Typed stale-shape diagnoses and a rolling success rate below `pool.rediscoverThreshold` also enter triage; arg, auth, `not_run`, `sent_unconfirmed`, and other non-repairable structural results terminate without automatic rediscovery. A caller reconciles an unconfirmed write through a separate read capability when possible. Warm benchmarks and CI checks use this graph to measure saved-strategy quality without silently treating HTTP 2xx as semantic success.
...
- **User-arbitration at save time.** Every `save_strategy` runs through the `user_confirmation` classifier in the save-strategy audit (`runtime/src/audit/lift/save-strategy.ts`). The classifier composes a 2–4 sentence summary of the proposed save (tier + endpoint + prereq count + page-script anchor type / recorded-path step count) and asks the user yes/no. The agent supplies `audit_answers.user_confirmation: {user_decision: "approve" | "reject", user_quote}`; on reject the save fails and the session **stays in the current phase** (DRIVE / TRIAGE / LIFT) — phase transitions are governed by their own surfaces (`submit_triage_plan`, `end_drive`, `end_drive`), never as a side effect of a save rejection. Test harnesses register a `SaveConfirmationDecider` that auto-resolves the prompt for autonomous runs; see `runtime/src/audit/lift/save-confirmation-decider.ts`.
