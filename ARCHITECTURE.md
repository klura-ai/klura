...
**Interruptions, remote viewing, and the save-strategy audit.** Login walls, CAPTCHAs, 2FA, ToS prompts, recorded-step failures, check-ins — anything that needs a mid-flow decision routes through a single plugin registry, and whichever handler claims it decides the resolution (resolved inline, continue silently, or hand off to a human via the remote viewer). The site sees the daemon's IP throughout — no fingerprint mismatch. Separately, every `save_strategy` call funnels through a single `Audit` class (`runtime/src/audit/`) that composes structural detectors and token-gated classifiers under one rejection envelope — adding a new save-time concern is one Detector or Classifier spec entry, not a new gate. See [docs/interruptions.md](docs/interruptions.md), [docs/remote.md](docs/remote.md), and [docs/gates.md](docs/gates.md).
...
| [docs/runtime.md](docs/runtime.md) | Daemon, tool surface (full tool inventory), agent loop, process architecture, drivers. |
...
| [docs/interruptions.md](docs/interruptions.md) | Plugin-routed mid-flow events — kind taxonomy, handler priority, `_interruption` envelope. |
...
The daemon is a lightweight orchestrator that can run on any machine. Sessions run as in-process Playwright — fast, no extra dependencies, with logical isolation between sessions on the same daemon. The daemon and the LLM don't have to be on the same machine — the daemon listens on a unix socket by default and TCP if you ask. The pool layer is built on `BrowserPool` / `BrowserDriver` abstract interfaces, so a future containerized or remote pool backend would slot in without the rest of the runtime seeing the difference.
